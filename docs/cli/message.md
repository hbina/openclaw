...
Single outbound command for sending messages and retained channel actions
...
- Values: `discord|telegram|whatsapp`
...
- For supported providers, channel names like `Help` or `#help` are resolved via the directory cache.
...
- `member info` (Discord): `--user-id`, `--guild-id`
