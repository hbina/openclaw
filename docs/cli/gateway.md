...
`gateway status` shows the Gateway service (launchd/systemd) plus an optional probe of connectivity/auth capability.
...
<AccordionGroup>
  <Accordion title="Status semantics">
    ...
    - `--deep` adds a best-effort scan for extra launchd/systemd installs. When multiple gateway-like services are detected, human output prints cleanup hints and warns that most setups should run one gateway per machine.
