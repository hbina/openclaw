...
- Extra launchd/systemd installs when `--deep` is used.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `Other gateway-like services detected (best effort)` → stale or parallel launchd/systemd units exist. Most setups should keep one gateway per machine; if you do need more than one, isolate ports + config/state/workspace. See [/gateway#multiple-gateways-same-host](/gateway#multiple-gateways-same-host).
